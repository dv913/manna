...
#define N_ROLLS 150000000UL  // number of experiments -- balls to drop
...
for (i = 0; i < count_full; i++) {
      ...
      int dist2 = genrand()*2;
       ...
       // define direction
      double  dir1 = genrand()*2;
      double  dir2 = genrand()*2;
      if(dir1 < 0.5) {dir1 = (int)-1;} else {dir1 = (int)1;}
      if(dir2 < 0.5) {dir2 = (int)-1;} else {dir2 = (int)1;}

      printf("\nBall 1 from cell %i moves: %i (%i,%i)", lattice[array_full[i]], (int)dist1*(int)dir1, (int)dist1, (int)dir1);
      printf("\nBall 2 from cell %i moves: %i  (%i,%i)\n", lattice[array_full[i]], (int)dist2*(int)dir2, (int)dist2,(int)dir2);
